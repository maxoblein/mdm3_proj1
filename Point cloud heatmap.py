...
def nn(inp, vec):
    ...
    #X = Initialise_3d(inp, [20, 40], [0.2, 0.25], [0, 0.012], [-0.05, 0.05], [0, 1150], [0,2])
    #x = Initialise_3d(inp, [20, 40], [0.2, 0.25], [0, 0.012], [-0.05, 0.05], [1151, 1289], [0,2])
    #XS = Initialise_3d_Swap(inp, [20, 40], [0.15, 0.2], [0, 0.02], [-0.05, 0], [0, 1150], [0,2])
    #xS = Initialise_3d_Swap(inp, [20, 40], [0.15, 0.2], [0, 0.02], [-0.05, 0], [1151, 1289], [0,2])
    #X = np.dstack((X,XS))
    #x = np.dstack((x,xS))
    plt.imshow(XS[:,:,573], origin = 'upper')
    
    X = X.reshape(1150, 21, 41, 1)
    x = x.reshape(138, 21, 41, 1)
    
    Y = to_categorical(vec[0:1150])
    y = to_categorical(vec[1151:1289])
    
    loaded_model = Sequential()
    
    #loaded_model = load_model('model.h5')
    
    loaded_model.add(Conv2D(64, kernel_size = 3, activation = 'relu', input_shape = (21,41,1)))
